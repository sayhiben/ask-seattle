...
If any turn creates or touches a billable resource, the agent must shut down the parts that create ongoing compute spend before ending the turn.

Treat this as higher priority than convenience, but do not destroy an explicitly requested warm cache just because it is billable.

Rules:

- never leave cloud compute billing between turns
...
- if a persistent billable cache is being preserved, say so explicitly in the final response
...
- confirm whether a persistent billable cache volume remains
- if a cache volume remains, state why it is being preserved and that this is intentional
